...
When a user clicks "Bind", the system must ensure that a submission isn't sent to the external provider multiple times simultaneously (e.g., if two users click the same button at the same time, or the user clicks the button from multiple tabs).
